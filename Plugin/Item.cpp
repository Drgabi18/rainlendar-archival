...
*/
/*
  $Header: //RAINBOX/cvsroot/Rainlendar/Plugin/Item.cpp,v 1.5 2003/10/27 17:37:50 Rainy Exp $
...
int CItem::GetDayType(int day, int month, int year)
{
	...
	for( ;  iter != eventList.end(); iter++)
	{
		...
		const Profile* profile = CConfig::Instance().GetProfile((*iter)->GetProfile().c_str());
