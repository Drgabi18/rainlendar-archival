...
*/
/*
  $Header: /home/cvsroot/Rainlendar/Plugin/EventListWindow.cpp,v 1.2 2004/12/05 18:32:14 rainy Exp $
...
{
	int a = arg1->GetStartTime().GetAsSeconds();
	int b = arg2->GetStartTime().GetAsSeconds();
...
**
*/
GUID* CEventListWindow::HitTest(int x, int y)
{
	RECT rc = GetEventListTextMargins();
	...
	for (int i = 0; i < (int)m_EventListItems.size(); i++)
	{
		...
		for (int j = 0; j < (int)m_EventListItems[i].guids.size(); j++)
		{
			...
			{
				return &(m_EventListItems[i].guids[j]);
			}
...
LRESULT CEventListWindow::OnLButtonDblClk(WPARAM wParam, LPARAM lParam) 
{
	...
	default:
		if (guid != NULL)
		{
			...
			if (event && !event->IsReadOnly())
			{
				OpenEditEventDialog(m_Window, CRainlendar::GetInstance(), event->GetGUID(), NULL);
			}
		}
	}
...
LRESULT CEventListWindow::OnMove(WPARAM wParam, LPARAM lParam) 
{
	...
	if(CConfig::Instance().GetMovable())
	{
		...
		CConfig::Instance().SetEventListX(x);
		CConfig::Instance().SetEventListY(y);
