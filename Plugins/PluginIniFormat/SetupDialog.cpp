...
*/
/*
  $Header: /home/cvsroot/Rainlendar/Plugins/PluginIniFormat/SetupDialog.cpp,v 1.1.1.1 2005/07/10 18:48:07 rainy Exp $
    ...
    { 
        ...
        case WM_COMMAND:
			switch (LOWORD(wParam)) 
			{
				...
				case IDOK:
					if ((HWND)lParam == GetDlgItem(hwndDlg, IDOK))
					{
        ...
        case WM_NOTIFY:
			{
				...
				else if(pNMHDR->code == LVN_ENDLABELEDIT)
				{
					...
					{
						HWND widget = GetDlgItem(hwndDlg, IDC_FILELIST);
						HWND widget2 = GetDlgItem(hwndDlg, IDC_FILELIST2);
						if (pNMHDR->hwndFrom == widget)
						{
							g_TmpEventFiles[pdi->item.iItem].filename = pdi->item.pszText;
						}
						else if (pNMHDR->hwndFrom == widget)
						{
							g_TmpTodoFiles[pdi->item.iItem].filename = pdi->item.pszText;
						}
						UpdateWidgets(hwndDlg);
						return TRUE;
					}
