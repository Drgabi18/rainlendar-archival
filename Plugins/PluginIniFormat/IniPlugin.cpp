...
/*
  ...
  Revision 1.4  2005/03/25 13:59:35  rainy
  no message

  Revision 1.3  2005/03/01 18:30:34  rainy
...
bool ImportEvents(LPCTSTR eventFile, bool readOnly, bool setID)
{
	...
	if (fileHandle != INVALID_HANDLE_VALUE)
	{
		...
		if (fileSize > 0 && fileSize != -1)
		{
			...
			DWORD readBytes;
			TCHAR* pBuffer = new TCHAR[fileSize + 30];
			memset(pBuffer, 0, sizeof(TCHAR) * (fileSize + 30));
			...
			if (ReadFile(fileHandle, pBuffer, fileSize, &readBytes, NULL))
			{
				...
				DebugLog("%s: Read %i events from file %s", PLUGIN_NAME, count, eventFile);
			}
			else
			{
				DebugLog("%s: Read error with file: %s", PLUGIN_NAME, eventFile);
			}
		...
		{
			DebugLog("%s: Unable to get the file size (%i).", PLUGIN_NAME, GetLastError());
		}
	...
	{
		DebugLog("%s: File not found: (%s).", PLUGIN_NAME, eventFile);
	}
...
bool ImportTodos(LPCTSTR todoFile, bool readOnly, bool setID)
{
	...
	if (fileHandle != INVALID_HANDLE_VALUE)
	{
		...
		if (fileSize > 0 && fileSize != -1)
		{
			...
			if (ReadFile(fileHandle, pBuffer, fileSize, &readBytes, NULL))
			{
				...
				DebugLog("%s: Read %i todos from file %s", PLUGIN_NAME, count, todoFile);
			}
			else
			{
				DebugLog("%s: Read error with file: %s", PLUGIN_NAME, todoFile);
			}
	...
	{
		DebugLog("%s: File not found: (%s).", PLUGIN_NAME, todoFile);
	}
