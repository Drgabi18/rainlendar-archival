...
/*
  ...
  Revision 1.30  2005/03/25 13:58:43  rainy
  no message

  Revision 1.29  2005/03/01 18:49:28  rainy
  Removed skin stuff.
...
void CConfig::ReadGeneralConfig(const char* iniFile)
{
	...
	m_WeekendDays=GetPrivateProfileInt( "Rainlendar", "WeekendDays", m_WeekendDays, iniFile);
...
void CConfig::WriteConfig(WRITE_FLAGS flags)
{
	...
	if (CompareIniTime(INIPath))
	{
		...
		if ((flags & WRITE_FULL) == WRITE_FULL)
		{
			...
			sprintf(tmpSz, "%i", m_WeekendDays);
			WritePrivateProfileString( "Rainlendar", "WeekendDays", tmpSz, INIPath.c_str() );
