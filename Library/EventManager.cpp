...
void CEventManager::Initialize(CPluginManager& pluginManager)
{
	...
	// Use default plugin first
	CPlugin* defPlugin = m_PluginManager->GetDefaultPlugin();
	if (defPlugin && defPlugin->IsEnabled()) 
	{
		defPlugin->ReadItems();
	}
...
bool CEventManager::FlushBufferedItems(bool sendToAll)
{
	...
	std::set<GUID, GUIDComparator> newItems;
	bool changed = false;
	UINT i, count;
		...
		for (i = 0; i < m_BufferedAddItems.size(); i++)
		{
			...
			if (item)
			{
				...
				if (item->GetTimeStamp() < m_BufferedAddItems[i]->GetTimeStamp())
				{
					...
					{
						newItems.insert(*m_BufferedAddItems[i]->GetGUID());
					}
	...
	if (changed)
	{
		...
		for (i = 0; i < allPlugins.size(); i++)
		{
			...
			std::set<GUID, GUIDComparator>::iterator iter = newItems.begin();
			for ( ; iter != newItems.end(); iter++)
			{
				CEntryItem* item = GetItem(&(*iter));
				if (item)
				{
					if (sendToAll || item->GetPluginID() != allPlugins[i]->GetPluginID())	// Don't include events that came from the plugin
					{
						if (!item->IsReadOnly())
						{
							guids[count++] = item->GetGUID();
						}
					}
				}
			}
