...
bool ExportEvents(LPCTSTR eventFile, RainlendarItem** items);
...
void Plugin_ItemNotification(GUID** guid)
{
	...
	for (i = 0; i < g_EventFiles.size(); i++)
	{
		...
		{
			Plugin_ExportItems(g_EventFiles[i].filename.c_str(), guid, RAINLENDAR_TYPE_EVENT);	// The type doesn't matter...
		}
	}
...
RainlendarEvent* ScanEvent(icalcomponent* component, bool readOnly)
{
//	DebugLog(icalcomponent_as_ical_string(component));	// DEBUG
	...
	if (catProp)
	{
		const char* cat = icalproperty_get_value_as_string(catProp);
		if (cat)
		{
...
RainlendarTodo* ScanTodo(icalcomponent* component, bool readOnly)
{
	const char* summary = icalcomponent_get_summary(component);
	const char* iCalGuid = icalcomponent_get_uid(component);
	icaltimetype timeStamp = icalcomponent_get_dtstamp(component);
...
int ScanEvent(icalcomponent* component, bool readOnly, bool setID, RAINLENDAR_TYPE type)
{
	if (type == RAINLENDAR_TYPE_EVENT)
	{
		RainlendarEvent* newEvent = ScanEvent(component, readOnly);
		if (newEvent)
		{
			if (!setID)
			{
	...
	{
		RainlendarTodo* newTodo = ScanTodo(component, readOnly);
		if (newTodo)
		{
			if (!setID)
			{
			...
			newTodo->position = 100;		// TODO: Fix this
...
bool ImportEvents(LPCTSTR eventFile, bool readOnly, bool setID, int items)
{
	...
	icalset* store = icalfileset_new(eventFile);
	if (store)
	{
		int eventCount = 0;
		int todoCount = 0;
...
void UpdateEventData(icalcomponent* component, RainlendarItem* item, bool newEvent)
{
	...
	if (item->type == RAINLENDAR_TYPE_EVENT)
	{
		...
		if (newEvent->profile && strlen(newEvent->profile) > 0)
		{
			...
			if (catProp)
			{
				icalproperty_set_categories(catProp, ConvertStringToACP(newEvent->profile));
			}
			else
			{
		...
		while (items[i] != NULL)
		{
			if (items[i] != (RainlendarItem*)-1)
			{
				if (!items[i]->deleted)
				{
					...
					else if (items[i]->type == RAINLENDAR_TYPE_TODO)
					{
						todoCount++;
...
void EndMonitor()
{
	if (g_Thread) 
	{
		// I really shouldn't use TerminateThread...
...
DWORD WINAPI MonitorThreadProc(LPVOID pParam)
{
	...
	std::vector<FILETIME> changeTimes;
    DWORD dwWaitStatus;
	char buffer[MAX_PATH];

	DebugLog("%s: Started file system monitor thread", PLUGIN_NAME);
	...
	for (int i = 0; i < size; i++)
	{
		...
		if (file)
		{
			...
			changeTimes.push_back(writeTime);
			CloseHandle(file);
...
//			DebugLog("%s: Something happened!", PLUGIN_NAME);	// DEBUG
			...
			if (dwWaitStatus >= WAIT_OBJECT_0 && dwWaitStatus < WAIT_OBJECT_0 + size)
			{
				...
				if (file)
				{
					...
					if (CompareFileTime(&writeTime, &changeTimes[i]) == 1)
					{
						DebugLog("%s: Noticed change in the file system -> Refresh", PLUGIN_NAME);

						changeTimes[i] = writeTime;
						Rainlendar_Refresh();		// Refresh Rainlendar
					}
				}
