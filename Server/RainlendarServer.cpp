...
*/
/*
  $Header: //RAINBOX/cvsroot/Rainlendar/Server/RainlendarServer.cpp,v 1.8 2003/05/25 18:15:32 Rainy Exp $
...
CRainlendarServer::CRainlendarServer(SockAddr& saBind, CIPFilter* filter)
	: SimpleServer(saBind)
{
	m_Busy = false;
	m_Combiner = NULL;
...
void CRainlendarServer::processSingleMsg(PacketMessage* pmsg)
{
	...
	case pcWaitingForEvents:
		{
			// Send the events
			std::list<PacketBuffer*>& packetList = m_Combiner->EncodePackets(m_Filter, psocket->getAddr().ipAddr());
