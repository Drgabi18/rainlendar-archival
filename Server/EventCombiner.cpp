...
#ifdef SERVER	// Only the server filters things
#include "IPFilter.h"
#endif
			...
			if (fgets(buffer, 1100, file) != NULL)
			{
				if (strlen(buffer) > 0)
				{
					...
					if (buffer[0] == '[')
					{
						...
						newEvent->everyNth = 0;
#ifdef SERVER
						newEvent->createdBy = 0;
						newEvent->lastModified = 0;
#endif
						date = buffer;
					}
					else
...
#ifdef SERVER
							else if (strncmp(buffer, "CreatedBy=", 10) == 0)
							{
								newEvent->createdBy = CIPNumber(buffer + 10);
							}
							else if (strncmp(buffer, "LastModified=", 13) == 0)
							{
								newEvent->lastModified = CIPNumber(buffer + 13);
							}
#endif
						}
					}
...
bool CEventCombiner::WriteEvents(const char* filename)
{
	...
	if (file)
	{
		...
		for ( ; i != m_Events.end(); i++)
		{
			...
			strcpy(buffer, "CreatedBy=");
			strcat(buffer, (*i).second->createdBy.GetString());
			strcat(buffer, "\n");
			fputs(buffer, file);

			strcpy(buffer, "LastModified=");
			strcat(buffer, (*i).second->lastModified.GetString());
			strcat(buffer, "\n");
			fputs(buffer, file);
		}
	...
	if (newEvent)
	{
		...
		if (i != m_Events.end())
		{
...
#ifdef SERVER
				newEvent->createdBy = m_Events[date]->createdBy;	// Preserve the created by value
#endif
			...
			else
			{
				...
				{
					char buffer[1024];
					sprintf(buffer, "Ignored the event %s because it's older than the existing one.", date.c_str());
					LOG(buffer);
					m_ErrorCode = ERROR_CODE_OUTOFDATE;
				}
...
std::list<PacketBuffer*>& CEventCombiner::EncodePackets(CIPFilter* filter, ULONG ipAddr)
{
	...
	for ( ; i != m_Events.end(); i++)
	{
		...
		if (filter)
		{
			if (!filter->CheckOutgoing(CIPNumber(ipAddr, 32), info->profile.c_str()))
			{
				 
				LOG("The outgoing event %s was filtered (rule: %s).", (*i).first.c_str(), filter->GetRule());
...
bool CEventCombiner::DecodePacket(PacketBuffer* packet, CIPFilter* filter, ULONG ipAddr)
{
	...
	if (filter)
	{
		if (!filter->CheckIncoming(CIPNumber(ipAddr, 32), newEvent->profile.c_str()))
		{
			LOG("The incoming event %s was filtered (rule: %s).", date.c_str(), filter->GetRule());
			return false;
		}
	}

	newEvent->lastModified = CIPNumber(ipAddr, 255);
#endif
