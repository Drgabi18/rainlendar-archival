...
*/
/*
  $Header: /home/cvsroot/Rainlendar/OutlookPlugin/OutlookPlugin.cpp,v 1.2 2004/12/05 18:14:35 rainy Exp $
		...
		if (SUCCEEDED(hr)) 
		{
			...
			if (propsFound)
			{
				...
				if (SUCCEEDED(hr)) 
				{
					...
					DebugLog("%s: Read %i events from Outlook", PLUGIN_NAME, count);
				}
				else
				{
					DebugLog("OUTLOOK: HrQueryAllRows failed: 0x%08x (%i)", hr, __LINE__);
...
void GetTodoData(LPMDB lpmdb, LPSPropValue value)
{
    HRESULT hr;
	LPMAPIFOLDER pfld = NULL;
    ULONG ulType;
		...
		if (SUCCEEDED(hr)) 
		{
			...
			if (propsFound)
			{
				...
				if (SUCCEEDED(hr)) 
				{
					...
					DebugLog("%s: Read %i tasks from Outlook", PLUGIN_NAME, count);
				}
				else
				{
					DebugLog("OUTLOOK: HrQueryAllRows failed: 0x%08x (%i)", hr, __LINE__);
...
//								DebugLog("Entry container: %s", pRowSet->aRow[i].lpProps[E_DATASTORE_CONTAINER].Value.lpszA);
								...
								{
									GetTodoData(lpmdb, &pRowSet->aRow[i].lpProps[E_DATASTORE_EID]);
								}
							}
						}
					}
					...
					for	(int i = 0; i < prows->cRows; i++)
					{
						...
						if (SUCCEEDED(hr))
						{
							...
							if (SUCCEEDED(hr))
							{
								...
								for (int i = 0; i < g_Stores.size(); i++)
								{
									if (g_Stores[i].name == pvalProp->Value.lpszA)
									{
										found = true;
										break;
									}
								...
								if (found || g_EnableAll)
								{
									ScanMessageFolders(lpmdb, (g_Stores.size()) > 0 ? &g_Stores[i] : NULL);
								}
